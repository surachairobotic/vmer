...
module.exports = {
  init: function(options = {}){
    ...
    module.exports.config = {
      ...
      "result_dir":PATH.resolve("./data/"),
      "route_dir":PATH.resolve("./route/"),
    };
    return check_dir_exist(module.exports.config.log_dir)
    .then(()=>{
      clear_old_files(module.exports.config.log_dir, 30);
  ...
  // not safe. can use glob pattern (*)
  // not work with the name with parenthesis like "data copy (2)"
  del: function(path, b_check_deleted = true){
    return new Promise((resolve,reject)=>{
      setTimeout(()=>{
        del( path, {force:true,dryRun:false})
        .then((eles)=>{
          if( b_check_deleted && eles.length!=path.length ){
  ...
  del_dir: function( dirs, b_check_deleted = true){
    ...
    return new Promise((resolve,reject)=>{
      setTimeout(()=>{
        Promise.all( dirs.map((d)=>{
          return fs.promises.rmdir( d, { recursive: true } )
          .then(resolve)
          .catch((e)=>{
            if( b_check_deleted )
              return reject('e');
...
function get_logfile_name(t=null){
  if( !t )
    t = logfile_time;
  //logfile_name = PATH.join( module.exports.config.log_dir, String(t.day)+".log" );
  logfile_name = PATH.join( module.exports.config.log_dir, String(t.year) + (t.month<10 ? '0' : '') + t.month
...
function clear_old_files(dir, day){
  ...
  .then((files)=>{
    return Promise.all( files.map((file)=>{
      ...
      .then((stat)=>{
        var endTime;
        endTime = new Date(stat.mtime).getTime() + day*24*3600000;
        if (now > endTime) {
          fs.unlink(PATH.join(dir, file), function(err) {
            if (err) {
              module.exports.log('error', 'clear_old_files() : fs.unlink failed : '+PATH.join(dir, file), err);
            }
            module.exports.log('info', 'clear_old_files() : remove : '+PATH.join(dir, file));
          });
  ...
  fs.readdir( dir, function(err, files) {
    ...
    files.forEach(function(file, index) {
      fs.stat(PATH.join(dir, file), function(err, stat) {
        ...
        if (err) {
          try{
            module.exports.log('error', 'clear_old_files() : fs.stat failed : '+PATH.join(dir, file), err);
          }
          catch(e){}
          return;
        }
