    ...
    .then((ret)=>{
      if( req.params.api=='upload_routes' ){
        ...
        .then((b_exist)=>{
          ...
          return my.del_dir(ret.dir, false);
        });
      }
      else{
...
function process_api( req, res, api_name, data ){
  if( api_name=='get_device_info' ){
    ...
    // remove all routes
    if( data.route_names=='*' ){
      return my.del_dir(my.config.route_dir, true)
      .then(()=>{
        return check_exist(my.config.route_dir);
      }).then((ret)=>{
        if( ret )
    ...
    return Promise.all( data.route_names.map( route_name =>{
      ...
      return Promise.all( data.route_names.map( route_name =>{
        return my.del_dir( PATH.resolve(PATH.join( route_dir, route_name )) );
      }));
    }).then(()=>{
      return get_route_list();
    }).then((routes)=>{
...
//    console.log(route_dir,path);
    ...
    .then((_files)=>{
      if( data.data_names=='*' ){
        ...
        my.log('warn', 'remove_data : data_names is not an array : '+JSON.stringify(data.data_names));
        return Promise.reject('Data names are not in an array');
      }
      else{
        files = data.data_names;
        for(var i=0;i<files.length;i++){
          if( !_files.includes(files[i]) ){
            my.log('warn', 'remove_data : file not found : '+files[i]);
            return Promise.reject('Data not found : '+files[i]);
          }
        }
      }
      if( files.length==0 ){
        my.log('info', 'remove_data : no result file');
        return Promise.reject('No data');
      }
      return my.del_dir( files.map( f => PATH.join(my.config.result_dir, f) ), true );
    }).then(()=>{
      return process_api( req, res, 'get_data_list', null );
    });
  }
  else{
