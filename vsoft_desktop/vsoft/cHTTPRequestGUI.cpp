...
cHTTPRequestGUI::cHTTPRequestGUI(MainWindow *_mainwindow, QString _projPath, cDB *_db)
    : mainwindow(_mainwindow)
    , progress_dialog(NULL)
    , projPath(_projPath)
    , db(_db)
{
    route_dir = projPath + "route/";
    data_dir = projPath + "data/";
    if(!QDir(route_dir).exists())
        QDir().mkdir(route_dir);
...
void cHTTPRequestGUI::create_route_tree(){
    mainwindow->ui->treeWidgetRoute_2->clear();
    QList<QTreeWidgetItem*> *routeWdgt = new QList<QTreeWidgetItem*>();
    db->get_routes(routeWdgt);
    commonFunction::setWidgetCheckState((*routeWdgt)[0], "Route");
    mainwindow->ui->treeWidgetRoute_2->addTopLevelItem((*routeWdgt)[0]);
}

void cHTTPRequestGUI::findCheckedRoute(QTreeWidgetItem *itm, QList<QString> *_route_dirs, QList<QString> *_routeJson, int checkState) {
    if(itm->whatsThis(0).contains("Route") && itm->checkState(0)==Qt::Checked) {
        _route_dirs->append(itm->text(0));
        cRouteWidget *routeWdgt = static_cast<cRouteWidget*>(itm);
        _routeJson->append(routeWdgt->cParent->desc);
    }
    for(int i=0; i<itm->childCount(); i++) {
        findCheckedRoute(itm->child(i), _route_dirs, _routeJson, checkState);
    }
}
...
void cHTTPRequestGUI::upload_routes(){
    ...
    QList<QString> route_jsons;
    findCheckedRoute(mainwindow->ui->treeWidgetRoute_2->topLevelItem(0), &route_dirs, &route_jsons, Qt::Checked);
    /*
    for( int i = 0; i < mainwindow->ui->treeWidgetRoute_2->topLevelItemCount(); ++i ) {
       QTreeWidgetItem *item_db = mainwindow->ui->treeWidgetRoute_2->topLevelItem( i );
    ...
    */
    if( route_dirs.isEmpty() ){
        ...
        for(int i=0; i<route_dirs.size(); i++) {
            qDebug() << route_dirs[i];
            if(QDir(route_dirs[i]).exists()) {
                commonFunction::rmDir(route_dir + route_dirs[i]);
            }
            QDir().mkdir(route_dir + route_dirs[i]);
            qDebug() << route_dir + route_dirs[i];
            qDebug() << route_dir + route_dirs[i] + "/" + route_dirs[i] + ".json";
            QFile file(route_dir + route_dirs[i] + "/" + route_dirs[i] + ".json");
            //file.open(QIODevice::WriteOnly);
            if(file.open(QIODevice::WriteOnly | QIODevice::Text)) {
                file.write("test\n");
                file.write(route_jsons[i].toStdString().c_str());
            }
            file.close();
            route_dirs[i] = route_dir + route_dirs[i];
        }
    }
