...
bool cHTTPRequest::download_data(const QString& ip_address, const QList<QString> &data_names, const QString& data_dir) {
    qDebug() << "cHTTPRequest::download_data";
    auto manager = new QNetworkAccessManager();
...
//            this->reply->deleteLater();
    ...
    qDebug() << "cHTTPRequest::download_data -- end";
    return true;
}
...
bool cHTTPRequest::extract(const QString& file_path, const QString& dst) {
    ...
    for (size_t i = 0; i < entries; ++i)
    {
        ...
        qDebug() << " unzip results [" << i << "] : " << p;
        if( commonFunction::isFile(p) ) {
            QString sub = p.section('/', 0, -2)+'/';
            qDebug() << "substring : " << sub;
            if( !dir.exists(sub) )
                dir.mkpath(sub);
        }
        else
            if( !dir.exists(p) )
                dir.mkpath(p);
        std::ofstream ofs(p.toStdString(), std::ios::binary);
        char buffer[1024];
        std::streamsize n;
